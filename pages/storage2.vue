...
const bodegaMainTable = ref(true);
...
async function handleVerBodegaClick(id) {
  bodegaMainTable.value = false;
  const WarehouseById = useAsyncData(async () => {
    const { data, error } = await supabase.rpc('get_pd_wh').eq('wid', id)
    console.log(data)
    return data
  })
}
                  ...
                  <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                    <div class="sm:flex sm:items-start">
                      ...
                      <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
                        ...
                        <div class="mt-2">
                          <p class="text-sm text-gray-500">¿Seguro que quieres borrar el o las bodegas seleccionadas?
                            Esta
    ...
    <div class="px-4">
      ...
      <div class="overflow-x-auto relative shadow-md sm:rounded-lg">
        <table class="w-full text-sm text-left text-gray-500">
          ...
          <tbody>
            <tr class="bg-white border-b hover:bg-gray-50" v-for="wh in WarehouseById" :key="wh.id">
              <td>{{ wh.id }}</td>
              <td>{{ wh.alias }}</td>
              <td>{{ wh.address }}</td>
              <td class="py-4 px-6">
                <a href="#" @click="handleVerBodegaClick(wh.id)" class="font-medium text-blue-600 hover:underline">Ver
                  Bodega</a>
              </td>
              <td class="py-4 px-6">
                <a href="#" type="button" @click="openDeleteWarehouseModal(wh.id)"
  ...
  <div class="px-4">
    <div class="overflow-x-auto relative shadow-md sm:rounded-lg">
        <table class="w-full text-sm text-left text-gray-500">
          ...
          <tbody>
            <tr class="bg-white border-b hover:bg-gray-50" v-for="wh in Pd_wh1" :key="wh.id">
              <td class="p-4 w-4">
                <div class="flex items-center">
                  <input id="checkbox-table-search-2" type="checkbox"
                    class="w-4 h-4 text-blue-600 bg-gray-100 rounded border-gray-300 focus:ring-blue-500 focus:ring-2">
    ...
    <div class="overflow-x-auto relative shadow-md sm:rounded-lg">
        <table class="w-full text-sm text-left text-gray-500">
          ...
          <tbody>
            <tr class="bg-white border-b hover:bg-gray-50" v-for="wh in Pd_wh2" :key="wh.id">
              <td class="p-4 w-4">
                <div class="flex items-center">
                  <input id="checkbox-table-search-2" type="checkbox"
                    class="w-4 h-4 text-blue-600 bg-gray-100 rounded border-gray-300 focus:ring-blue-500 focus:ring-2">
